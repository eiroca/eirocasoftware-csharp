...
namespace Foxoft.Ci {
	...
	public class GenPas : SourceGenerator, ICiSymbolVisitor {
		...
		public class SymbolMapping {
			...
			private static Dictionary<CiSymbol, SymbolMapping> varMap = new  Dictionary<CiSymbol, SymbolMapping>();
			
			public static SymbolMapping Find(CiSymbol symbol) {
			...
			static private string[] PascalWords = new String[] {"absolute", "and", "array", "as", "asm", "begin", "case", "class", "const", "constructor", "destructor", "dispinterface", "div", "do", "downto", "else", "end", "except", "exports", "file", "finalization", "finally", "for", "function", "goto", "if", "implementation", "in", "inherited", "initialization", "inline", "inline", "interface", "is", "label", "library", "mod", "nil", "not", "object", "of", "on", "on", "operator", "or", "out", "packed", "packed", "procedure", "program", "property", "raise", "record", "reintroduce", "repeat", "resourcestring", "self", "set", "shl", "shr", "string", "then", "threadvar", "to", "try", "type", "unit", "until", "uses", "var", "while", "with", "xor", "result", "length" };
			...
			public string GetUniqueName(bool inParentCheck) {
				...
				StringBuilder tmpName = new StringBuilder(this.Symbol.Name.Length);
				foreach (char c in this.Symbol.Name) {
			...
			public static SymbolMapping AddSymbol(SymbolMapping aParent, CiSymbol aSymbol) {
				return AddSymbol(aParent, aSymbol, true);
			}
			
			public static SymbolMapping AddSymbol(SymbolMapping aParent, CiSymbol aSymbol, bool inParentCheck) {
		...
		public class ClassOrder {
			private static List<CiClass> order = new List<CiClass>();
			
			public static List<CiClass>GetList() {
				return order;
			}
			
			public static void AddClass(CiClass klass) {
				if (klass==null) {
					return;
				}
		...
		public class SuperType {
			private static HashSet<CiClass> refClass = new HashSet<CiClass>();
			private static HashSet<CiType> refType = new HashSet<CiType>();
			
			private static Dictionary<CiType, TypeMappingInfo> TypeCache = new Dictionary<CiType, TypeMappingInfo>();
			
			public static HashSet<CiClass>GetClassList() {
				return refClass;
			}
			
			public static HashSet<CiType>GetTypeList() {
				return refType;
			}

			public static string GetTypeName(CiType type) {
				return GetTypeInfo(type).Name;
			}
			
			public static TypeMappingInfo GetTypeInfo(CiType type) {
			...
			public static TypeMappingInfo DecodeType(CiType type) {
			...
			public static void AddType(CiType type) {
				if (type==null) {
					return;
				}
		...
		public class NoIIFExpand {
			...
			private static Stack<int> instructions = new Stack<int>();
			
			public static void Reset() {
				instructions.Clear();
			}
			
			public static void Push(int step) {
			...
			public static bool In(int inst) {
		...
		public class MethodStack {
			...
			private static Stack<CiMethod> methods = new Stack<CiMethod>();
			
			public static void Reset() {
				methods.Clear();
			}
			
			public static void Push(CiMethod call) {
				methods.Push(call);
			}
			public static void Pop() {
				methods.Pop();
		...
		public class ExprType {
			private static Dictionary<CiExpr, CiType> exprMap = new  Dictionary<CiExpr, CiType>();
			
			public static void Reset() {
				exprMap.Clear();
			}
			
			public static CiType Get(CiExpr expr) {
				CiType result;
			...
			public static CiType Analyze(CiExpr expr) {
				if (expr==null) return CiType.Null;
		...
		public class BreakExit {
			private static Dictionary<ICiStatement, BreakExit> mapping = new  Dictionary<ICiStatement, BreakExit>();
			private static Dictionary<CiMethod, List<BreakExit>> methods = new Dictionary<CiMethod, List<BreakExit>>();
			
			private static Stack<BreakExit> exitPoints = new Stack<BreakExit>();
			...
			public static void AddSwitch(CiMethod method, CiSwitch aSymbol) {
			...
			public static List<BreakExit> GetLabels(CiMethod method) {
				List<BreakExit> labels = null;
				methods.TryGetValue(method, out labels);
				return labels;
			}

			public static BreakExit GetLabel(ICiStatement stmt) {
			...
			public static BreakExit Push(ICiStatement stmt) {
		...
		string Namespace;
		ProgramPreProcessing prePro = new ProgramPreProcessing();
...
");
		...
		void WriteMethodIntf(CiMethod method) {
			...
			switch (method.CallType) {
					case CiCallType.Abstract: Write("; virtual; abstract"); break;
		...
		protected override void Write(CiUnaryExpr expr) {
			switch (expr.Op) {
					case CiToken.Increment: Write("__CINC_Pre("); break;
					case CiToken.Decrement: Write("__CDEC_Pre("); break;
					case CiToken.Minus: Write("-("); break;
					case CiToken.Not: Write("not ("); break;
		...
		protected override void Write(CiPostfixExpr expr) {
			switch (expr.Op) {
					case CiToken.Increment: Write("__CINC_Post("); break;
					case CiToken.Decrement: Write("__CDEC_Post("); break;
					default: throw new ArgumentException(expr.Op.ToString());
			}
			WriteChild(expr, expr.Inner);
			Write(")");
		...
		protected override void WriteOp(CiBinaryExpr expr) {
			switch (expr.Op) {
					...
					case CiToken.Slash: WriteDiv(ExprType.Get(expr)); break;
					case CiToken.Mod: Write(" mod "); break;
					case CiToken.ShiftLeft: Write(" shl "); break;
					case CiToken.ShiftRight: Write(" shr "); break;
					...
					case CiToken.And: Write(" and "); break;
					case CiToken.Or: Write(" or "); break;
					case CiToken.Xor: Write(" xor "); break;
					case CiToken.CondAnd: Write(" and "); break;
					case CiToken.CondOr: Write(" or "); break;
					default: throw new ArgumentException(expr.Op.ToString());
			}
		}
		...
		public void WriteAssign(CiExpr Target, CiToken Op) {
			...
			if (Op!=CiToken.Assign) {
				...
				switch (Op) {
						...
						case CiToken.DivAssign: WriteDiv(ExprType.Get(Target)); break;
						case CiToken.ModAssign: Write(" mod "); break;
						case CiToken.ShiftLeftAssign: Write(" shl "); break;
						case CiToken.ShiftRightAssign: Write(" shr "); break;
						case CiToken.AndAssign: Write(" and "); break;
						case CiToken.OrAssign: Write(" or "); break;
						case CiToken.XorAssign: Write(" xor "); break;
